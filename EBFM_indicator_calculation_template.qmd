...
mainDir <- "directory_where_species_qmd_and_species_ID_and_fleet_ID_files_kept"

# The Ecosim output file directory - this allows for a direcotry with multiple sub directories
InputDir <- "modelOutput_directory"
InputsubDir <- "directory_in_EwE_output_directory_with_scenario_output"

# Sub diretory that contains the simulation with No Fishing. If you are using the burn-in option this will never be used
BosubDir <- 'directory_with_no_fishing_run_output'
...
OutsubDirStep1 <- "top_directory_for_output" 
OutsubDirStep2 <- "sub_directory_where_you_want_r_script_plots_stored_as_saved" # This is where plots will be saved.
```
...
consumption_filename <- "consumption_from EwE.csv"
diet_filename <- "diet_from EwE.csv"
```
...
bo_in_same_file <- 1 # takes bo from the biomass_annual.csv file - using the row set in bo_yr_row
...
bo_yr_row <- 99   # Row used to set the unfished Bo value if in the same file
...
yr_ignore <- 1950 # If using a burn in period this is the year before the historical time series begins - plots will filter out (not show) years before this time 

# Area of Ecopath model domain in km2
area_model <- 10000040
